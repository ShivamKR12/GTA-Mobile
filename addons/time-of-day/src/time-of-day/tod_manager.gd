...
func _process(delta: float) -> void:
	if editor_hint and !run_in_editor:
		return
	if !system_sync:
		_time_process(delta)
		_repeat_full_cucle()
...
var street_lights_on := false
var first_lights_check := false

func _time_process(p_delta) -> void:
	if time_cycle_duration != 0.0:
		timeline = timeline + p_delta / time_cycle_duration * 24.0
		RenderingServer.global_shader_parameter_set("Time", timeline)
		if (!street_lights_on and (timeline < 5 or timeline > 17)) or (street_lights_on and timeline > 5 and timeline < 17):
			street_lights_on = !street_lights_on
			for x in get_tree().get_nodes_in_group("Night Lights"): x.visible = street_lights_on
		elif first_lights_check: street_lights_on = !street_lights_on
		first_lights_check = true
